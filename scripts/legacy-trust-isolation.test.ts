...
test.skipIf(!certutil || !openssl)("legacy NSS verification detects removal and trust changes in a disposable database", () => {
  ...
  const verify = () => run(["bash", join(root, ".github/scripts/assert-nss-anchor.sh"), database, nickname, certificate]);
  mkdirSync(database);
  try {
    mustRun([openssl!, "req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1", "-subj", `/CN=${legacy.caCommonName}`, "-addext", "basicConstraints=critical,CA:TRUE", "-addext", "keyUsage=critical,keyCertSign,cRLSign", "-keyout", join(directory, "ca-key.pem"), "-out", certificate]);
    mustRun([certutil!, "-N", "--empty-password", "-d", `sql:${database}`]);
    mustRun([certutil!, "-A", "-d", `sql:${database}`, "-n", nickname, "-t", "C,,", "-i", certificate]);
    ...
    mustRun([openssl!, "req", "-x509", "-new", "-key", join(directory, "ca-key.pem"), "-days", "1", "-subj", "/CN=Different test anchor", "-addext", "basicConstraints=critical,CA:TRUE", "-out", replacement]);
    mustRun([certutil!, "-D", "-d", `sql:${database}`, "-n", nickname]);
    mustRun([certutil!, "-A", "-d", `sql:${database}`, "-n", nickname, "-t", "C,,", "-i", replacement]);
