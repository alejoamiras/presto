...
const workflow = Bun.YAML.parse(
  readFileSync(join(root, ".github/workflows/presto-previews.yml"), "utf8"),
) as {
  jobs: { upload: { steps: Array<{ name?: string; run?: string; uses?: string }> } };
};
const writer = workflow.jobs.upload.steps.find(
  (step) => step.name === "Write fixed preview configuration",
)?.run;
...
test("credentialed previews generate fixed SPA config without reading PR code", () => {
  ...
  expect(
    workflow.jobs.upload.steps.some((step) => step.uses?.startsWith("actions/checkout@")),
  ).toBe(false);
  ...
  try {
    for (const site of ["landing", "playground"]) {
      ...
      const production = JSON.parse(
        readFileSync(join(root, `packages/${site}/wrangler.jsonc`), "utf8"),
      );
      expect(config).toEqual({
        ...
        assets: {
          ...
          not_found_handling: production.assets.not_found_handling,
        },
      });
