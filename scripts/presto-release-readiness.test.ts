...
const key = readFileSync(new URL("../packages/presto/core/tests/fixtures/updater/pubkey.b64", import.meta.url), "utf8").trim();
...
test("release identity binds the domain, native identifier, endpoint and recovery key", () => {
  ...
  expect(() => assertReleaseIdentity(identity, { ...config, identifier: "invalid.pending-domain.presto" })).toThrow("identifier");
  expect(() => assertReleaseIdentity({ ...identity, recoveredUpdaterPublicKey: null }, config)).toThrow("recovery");
  expect(() => assertReleaseIdentity({ ...identity, releaseReady: false }, config)).toThrow("readiness");
});
...
test("native and npm release entry points enforce readiness before downstream jobs", async () => {
  for (const file of ["release-presto.yml", "release-sdk.yml"]) {
    const workflow = await Bun.file(new URL(`../.github/workflows/${file}`, import.meta.url)).text();
...
test("feed promotion uses only the fresh namespace from Wrangler configuration", async () => {
  const workflow = await Bun.file(new URL("../.github/workflows/release-presto.yml", import.meta.url)).text();
  const wrangler = await Bun.file(new URL("../packages/release-feed/wrangler.jsonc", import.meta.url)).text();
  const namespace = /"id":\s*"([a-f0-9]{32})"/.exec(wrangler)?.[1];
  expect(namespace).toBeDefined();
  const references = [...workflow.matchAll(/--namespace-id ([a-f0-9]{32})/g)].map((match) => match[1]);
