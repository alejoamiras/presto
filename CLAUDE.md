...
- **Testing**: 25 WebDriver E2E tests (macOS + Linux + Windows) via `tauri-plugin-webdriver` + WebdriverIO (incl. a real native UltraHonk proof of the committed Noir fixture), 66 Playwright UI mock tests, ~445 Rust tests (434 non-ignored on Linux; plus `win_acl` inline DACL tests on the windows-build lane), ~390 TS unit tests (SDK 107 + presto scripts 105 + root scripts 125 + playground 55), plus `#[ignore]`d integration suites run in dedicated CI lanes (`trust_*`, `autostart_heal`, `uninstall_ownership` per OS; `ultra_honk_real_bb` with the real bb, followed by an HTTP smoke against two coexisting headless instances). Noir fixtures live in `fixtures/noir/` (bb.js WASM references; `scripts/noir-fixture.ts --verify`). WebDriver tests run as PR gate and pre-release gate.
