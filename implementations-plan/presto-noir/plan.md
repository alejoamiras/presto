...
status: APPROVED 2026-09-07 (owner verdict: approve; A-01..A-07 resolved — see Assumptions → Asks); implementation not started
...
| `core/src/bb.rs` | Extract `run_bb(cmd, timeout) -> io::Result<(ExitStatus, stderr_tail)>` from `prove_with_timeout` (spawn_and_register → capped drain → wait → `guard.finish()` → stderr log); `prove_with_timeout` becomes workspace + args + `run_bb` + read + header. New `prove_ultra_honk(job, version, threads) -> UltraHonkOutput { proof, public_inputs, vk: Option<Vec<u8>> }` with the same lease/tempdir/containment/timeout (`PROVE_TIMEOUT` 300 s), same `prove-` prefix and reaper. |
...
| `core/src/server/auth.rs` | `authorize_origin` returns the `Approval` above (origin it already parses, `None` for no-Origin/`--allow-all`, plus the generation); callers ignoring it are unchanged. |
...
**Phase 2 — `bb.rs`: `run_bb`, `VerifierTarget`, `prove_ultra_honk`.** Extract the runner (chonk tests unchanged), add the sibling function and its fake-bb tests: exact argv for `-k` vs `--write_vk`, three input files written 0600, `proof`/`public_inputs`/`vk` read with caps, empty `public_inputs` accepted, missing output → error, oversize → error, timeout → kill tree.
...
**Phase 3 — `prove.rs` split into `admit` / `acquire_prover`.** Behaviour-preserving for chonk (`admit(Chonk)` takes no origin slot; no approval re-check).
...
**Phase 17 — Noir panel.** `packages/playground/src/noir.ts` (+ test): load `fixtures/noir/square` via `new URL(..., import.meta.url)`, `PrestoUltraHonkBackend` seeded with the fixture VK, WASM-vs-Presto proving with timings and a "proof identical to fixture" check, reusing `appendLog`/`setStatus`/`SparkOrbitController` (`AnimationPhase` + `noir:*`); `index.html` section; direct `@aztec/bb.js@5.2.0` + `@alejoamiras/presto-noir` (`workspace:*`); `vite.config.ts` `resolve.dedupe` for `@aztec/bb.js`; `app.yml` paths gain `packages/sdk-noir/**` and `fixtures/noir/**`; the playground's direct `@aztec/bb.js` and `packages/sdk-noir/package.json` join `update-aztec-version.ts`'s lockstep set.
...
- F-01 `/prove` hands the raw body to bb: `packages/presto/core/src/server/prove.rs:348`; guard sequence and drop order `prove.rs:224-379`.
