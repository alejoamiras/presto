...
pub struct PrestoConfig {
    ...
    /// Whether the local HTTPS listener (browser ⇄ presto encrypted channel) is enabled.
...
pub fn load_from(path: &std::path::Path) -> PrestoConfig {
...
pub struct ConfigStore {
    pub lock: parking_lot::RwLock<PrestoConfig>,
    pub cap: Option<PersistCapability>,
}

impl std::ops::Deref for ConfigStore {
    type Target = parking_lot::RwLock<PrestoConfig>;
...
impl ConfigStore {
    ...
    #[cfg(test)]
    pub(crate) fn for_test(config: PrestoConfig) -> Self {
        Self {
...
pub fn lock_mutate_save(
    lock: &parking_lot::RwLock<PrestoConfig>,
    cap: &PersistCapability,
    f: impl FnOnce(&mut PrestoConfig) -> bool,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
/// DEVELOPER'S OWN `~/.presto/config.json` — and an assertion against the in-memory config
...
mod tests {
    ...
    fn default_config_has_https_disabled() {
        let config = PrestoConfig::default();
    ...
    fn mutate_leaves_memory_unchanged_when_save_refused() {
        ...
        std::fs::write(&path, r#"{"config_version": 999}"#).unwrap();
        let lock = parking_lot::RwLock::new(PrestoConfig::default());
        let result =
    ...
    fn config_roundtrip_via_save_load() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let cfg_dir = dir.path().join(".presto");
    ...
    fn legacy_safari_support_key_is_ignored_https_defaults_off() {
        ...
        let cfg: PrestoConfig = serde_json::from_str(r#"{"safari_support": true}"#).unwrap();
        assert!(
            !cfg.https_enabled,
    ...
    fn config_roundtrip_auto_update_none() {
        ...
        let loaded: PrestoConfig = serde_json::from_str(&json).unwrap();
    ...
    fn load_returns_default_for_missing_file() {
        let config: PrestoConfig = serde_json::from_str("{}").unwrap_or_default();
        assert!(!config.https_enabled);
    }

    #[test]
    fn load_returns_default_for_malformed_json() {
        let config: PrestoConfig = serde_json::from_str("not json").unwrap_or_default();
        assert!(!config.https_enabled);
    }
    ...
    fn auto_update_missing_deserializes_as_none() {
        let config: PrestoConfig = serde_json::from_str("{}").unwrap();
        assert_eq!(config.auto_update, None);
    }

    #[test]
    fn approved_origins_removal() {
        let mut config = PrestoConfig {
    ...
    fn theme_roundtrips_through_the_lowercase_wire_form() {
        ...
        let cfg: PrestoConfig = serde_json::from_str(r#"{"theme":"dark"}"#).unwrap();
