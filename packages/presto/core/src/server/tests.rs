...
async fn health_hides_https_port_when_https_configured_but_not_bound() {
    ...
    let cfg = crate::config::PrestoConfig {
        https_enabled: true,
        ..Default::default()
    };
    let state = AppState {
...
fn auth_state_with_popup(
    ...
    // `config::config_path()`, i.e. the developer's real `~/.presto/config.json`.
...
/// `~/.presto/config.json` (post-impl codex).
fn auth_state_with_popup_at(
    ...
    let cfg = crate::config::PrestoConfig::default();
    let state = AppState {
        core: Arc::new(HeadlessState {
