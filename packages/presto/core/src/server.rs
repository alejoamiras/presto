...
pub const PORT_CONFLICT_GUIDANCE: &str =
    "Port 59833 is in use. Quit the other local prover, then restart Presto.";
...
/// The server-side core state — everything the headless `presto-server` needs, with no GUI
...
/// most "an presto exists", not its version or cached set. After SEC-01a every caller already
