...
fn mutate_config(config: &ConfigState, f: impl FnOnce(&mut PrestoConfig)) -> Result<(), String> {
    ...
    if serving_stale_identity {
        ...
        return Err(
            "HTTPS is set up, but the running server is still using a previous \
                    certificate. Restart Presto to finish enabling it."
                .to_string(),
        );
    }
