import type { PrestoStatus, PrestoStatusCheckOptions } from "@alejoamiras/presto";
...
interface PrestoStatusControllerOptions {
  ...
  render: (status: PrestoStatus) => void;
...
export class HttpSessionConsentController {
  ...
  confirm(): Promise<void> {
    ...
    this.options.setConfirmationOpen(false);
    this.options.setPending(true);
    this.options.announce("Enabling HTTP for this tab and checking Presto status.");
    ...
    const operation = Promise.resolve()
      ...
      .then(() => {
        this.options.announce("HTTP is allowed for this tab only. Presto status refreshed.");
      })
      .catch((error) => {
