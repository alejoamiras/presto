...
describe("prestoStatusView", () => {
  test("renders every unavailable reason without contradictory install UI", () => {
    const offline = prestoStatusView({ available: false, reason: "offline" });
...
describe("PrestoStatusController", () => {
  test("a stale result cannot overwrite a newer forced refresh", async () => {
    ...
    const first = new Promise<PrestoStatus>((resolve) => {
      resolveFirst = resolve;
    });
    const recovered: PrestoStatus = {
      available: true,
      needsDownload: false,
      protocol: "http",
    };
    let calls = 0;
    const rendered: PrestoStatus[] = [];
  ...
  test("proof fallback refresh is coalesced and only starts after available was displayed", async () => {
    let releases = 0;
    let resolveRefresh!: (status: PrestoStatus) => void;
    ...
    const controller = new PrestoStatusController({
      check: async () => {
        ...
        return new Promise<PrestoStatus>((resolve) => {
  ...
  test("permission change waits out the old SDK single-flight before forcing a fresh probe", async () => {
    let resolveStartup!: (status: PrestoStatus) => void;
    const startupStatus = new Promise<PrestoStatus>((resolve) => {
